...
from src.backtesting.vibe_engine.metrics import (
...
def _load_optimizer(config: Dict[str, Any]) -> Optional[Callable]:
    ...
    try:
        mod = importlib.import_module(f"src.backtesting.vibe_engine.optimizers.{opt_name}")
...
class BaseEngine(ABC):
    ...
    def run_backtest(
        ...
        # Benchmark = equal-weight basket of the scanned symbols. The upstream
        # project supported an external benchmark ticker here (backtest.benchmark);
        # that module was not ported, so config["benchmark"] is ignored.
        ...
        from src.backtesting.vibe_engine.run_card import write_run_card
        write_run_card(
            run_dir,
            config,
            m,
        ...
        # Log scalar metrics (skip nested dicts for JSON compat). Upstream
        # printed to stdout; this runs inside a service, so use the logger.
        logger.info(
            "Backtest metrics: %s",
            json.dumps({k: v for k, v in m.items() if not isinstance(v, dict)}),
        )
        return m
